...
{
  int mutex_num;
  mutex_num = pcb[run_pid].proc_frame_p->EBX;
  if(mutex[mutex_num].lock==LOCK)//locked and locked
  {
    EnQ(run_pid,&mutex[mutex_num].wait_q);
    pcb[run_pid].state = WAIT;
    run_pid = -1;
  }else
  {
    //EnQ(run_pid,&run_q);
    mutex[mutex_num].lock = LOCK;
  }
...
void MutexUnlockHandler(void)
{
  ...
  if(mutex[mutex_num].wait_q.size ==0)
  {
    mutex[mutex_num].lock = UNLOCK;

  }else
  {  
    pid = DeQ(&mutex[mutex_num].wait_q);
    pcb[pid].state = RUN;
    EnQ(pid,&run_q);
...
void ForkHandler(proc_frame_t *parent_frame_p)
{
   ...
   //should prob have sigchld stuff here
   
   
   pcb[child_pid].cr3 = pcb[run_pid].cr3;
...
void SignalHandler(proc_frame_t *proc_frame_p){
   ...
   {
      pcb[run_pid].sigchld_handler =(func_p_t)proc_frame_p -> ECX;
   }   
}
...
void ExitHandler(proc_frame_t *p)
{
   ...
   if(pcb[ppid].state != WAITCHLD)
   {
      ...
      if(pcb[ppid].sigchld_handler != NULL)//null?
      {
         
         InsertWrapper(ppid, pcb[ppid].sigchld_handler);//two things?parent or child? which function?
      }
   }else {
      
      //release parent, change state to ?  RUN?
      pcb[ppid].state = RUN;
      //enqueue it to?                     run_q?
      EnQ(ppid, &run_q);//is parent in wait q?

      //do'nt forget to deliver to the released parent process:
      //1. child's exiting number
      parent_exit_num_p = (int *) pcb[ppid].proc_frame_p->EBX;
      *parent_exit_num_p =  pcb[run_pid].proc_frame_p->EBX;
      //2. exiting child PID
      pcb[ppid].proc_frame_p->ECX = run_pid;
      //kernel reclaims exiting child process' resources:
      //1. its PID (enqueue it to ?)
      EnQ(run_pid,&ready_q);
      //2. its state (changed back to ?)
...
{
   int child_pid, *parent_exit_num_p,child_exit_num;
   int i;
   for(i=0;i<PROC_NUM;i++)
   {
   ...
   }else
   {
      ...
      //1.ZOMBIE's exiting number      in ebx
...
void ExecHandler(proc_frame_t *p)
{
   ...
   MyMemcpy((char *)page[main].addr, (char *)p->EBX, 4096);
   pcb[run_pid].proc_frame_p = (proc_frame_t *)(page[main].addr + PAGE_SIZE);
   pcb[run_pid].proc_frame_p--;
   MyMemcpy((char *)pcb[run_pid].proc_frame_p, (char *)p,sizeof(proc_frame_t));
   pcb[run_pid].proc_frame_p->EIP = (unsigned int)page[main].addr;
   set_cr3(pcb[run_pid].cr3);
