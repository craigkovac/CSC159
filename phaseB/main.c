...
unsigned int mainTable[PAGE_SIZE];
unsigned int subTable1[PAGE_SIZE];
unsigned int subTable2[PAGE_SIZE];
unsigned int instructionTable[PAGE_SIZE];
unsigned int stackTable[PAGE_SIZE];
...
void InitTerms(int port)
{
   ...
   for(i=0;i<50000;i++) asm("inb $0x80");   
   outportb(port + DATA, '[');
   for(i=0;i<50000;i++) asm("inb $0x80");   
...
int main(void) {  // OS bootstraps
   ...
   //int oerm_kb_wait_q[2];
   //int term_screen_wait_q[2];

   MyBzero((char*) &term_kb_wait_q[0],sizeof(q_t));
   MyBzero((char*) &term_kb_wait_q[1],sizeof(q_t));

   MyBzero((char*) &term_screen_wait_q[0],sizeof(q_t));
   MyBzero((char*) &term_screen_wait_q[1],sizeof(q_t));
   ...
   pies = 0;
   mutex[0].wait_q.size = 0;
   mutex[1].wait_q.size = 0;
   mutex[2].wait_q.size = 0;
   mutex[3].wait_q.size = 0;
   mutex[0].lock = UNLOCK;
   mutex[1].lock = UNLOCK;
   mutex[2].lock = UNLOCK;
   mutex[3].lock = UNLOCK;
   ...
   MyBzero((char *)&run_q,sizeof(q_t));//making it 20 because of below enqueue line

   MyBzero((char *)&terminal_wait_queue[0],sizeof(q_t));
   MyBzero((char *)&terminal_wait_queue[1],sizeof(q_t));
   MyBzero((char *)&terminal_buffer[0], sizeof(q_t));
   MyBzero((char *)&terminal_buffer[1], sizeof(q_t));
   ...
   for(q=0;q<PAGE_NUM;q++)
   {
      //page[q].addr = 0xe00000 + 0x1000 * i;
      page[q].addr = 0xe00000 + 4096 * q;
      page[q].LRU = 0;
      page[q].pid = -1;
   }
...
void Kernel(proc_frame_t *proc_frame_p) {   // kernel code runs (100 times/second)
   ...
   else if( proc_frame_p->event_type == SYSCALL_EVENT )
   {
      switch(pcb[run_pid].proc_frame_p->EAX )
      {
         ...
         case FORK:
            ForkHandler(pcb[run_pid].proc_frame_p);
            
         break;
         
         case SIGNAL:
            SignalHandler(pcb[run_pid].proc_frame_p);
         break;
